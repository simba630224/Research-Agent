...
這個專案可用 GitHub Actions 每日執行，將台股市值前二十名與美股市值前十名的價格、技術指標、公開新聞／研究資訊連結、50 字內摘要及**規則式資訊參考**寫入 Google Sheet。市值只用來排序，不會寫入報表。
...
| 技術訊號 | 判斷條件 | 規則式資訊參考 |
| --- | --- | --- |
| 偏多 | 收盤價 > MA20 > MA50、MACD > 0、RSI < 70 | 關注：趨勢偏多且未過熱。 |
| 偏空／過熱 | 收盤價 < MA20 < MA50 且 MACD < 0，或 RSI > 75 | 保守：趨勢偏弱或可能過熱。 |
| 盤整 | 不符合前兩種條件 | 觀望：訊號未一致。 |
